...
function ViewCartItems() {
  ...
  const updateQuantity = async (cartItemId, newQuantity) => {
    try {
      ...
      setCartItems((prevItems) =>
        prevItems.map((item) =>
          item._id === cartItemId
            ? { ...item, quantity: newQuantity, totalPrice: item.menuItemDetails.price * newQuantity }
            : item
        )
      );
    } catch (error) {
  ...
  return (
    <div className="p-6 min-h-screen bg-gray-100">
      ...
      {/* Subtotal Section */}
      {cartItems.length > 0 && (
        <div className="mt-6 p-4 bg-white shadow-md rounded-lg flex justify-between items-center">
